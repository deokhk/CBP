...
def parse_option():
    ...
    parser.add_argument('--sg_train_filepath', type = str, default = "data/crosslingual_pt_oracle_zh/sql_generation_train.json",
                        help = 'file path of sql generation training set.')
    parser.add_argument('--sp_train_filepath', type = str, default = "data/crosslingual_pt_oracle_zh/schema_prediction_train.json",
                        help = 'file path of schema prediction training set.')
    parser.add_argument('--vp_train_filepath', type = str, default = "data/crosslingual_pt_oracle_zh/value_prediction_train.json",
                        help = 'file path of value prediction training set.')
    parser.add_argument('--sg_val_filepath', type = str, default = "data/crosslingual_pt_oracle_zh/sql_generation_dev.json",
                    help = 'file path of machine translation training set.')
    parser.add_argument('--sp_val_filepath', type = str, default = "data/crosslingual_pt_oracle_zh/schema_prediction_val.json",
                        help = 'file path of schema prediction training set.')
    parser.add_argument('--vp_val_filepath', type = str, default = "data/crosslingual_pt_oracle_zh/value_prediction_val.json",
                        help = 'file path of value prediction training set.')
...
def train(opt):
    ...
    if opt.wandb_log and is_local_main_process:
        wandb_exp_name = opt.exp_name if opt.exp_name else f"{opt.model_name_or_path}_crosslingual_pretrain"
        wandb.init(
            project="ZX_seq2seq",
            name=wandb_exp_name,
            config=opt
    ...
    with accelerator.main_process_first():
        train_sg_dataset = datasets.Dataset.from_json(opt.sg_train_filepath)
        train_sp_dataset = datasets.Dataset.from_json(opt.sp_train_filepath)
        train_vp_dataset = datasets.Dataset.from_json(opt.vp_train_filepath)
        # drop unnecessary columns
        train_sg_dataset = train_sg_dataset.remove_columns(["db_id", "tc_original"])
        train_sp_dataset = train_sp_dataset.remove_columns(["mentioned_schema_items"])
        train_vp_dataset = train_vp_dataset.remove_columns(["mentioned_values"])

        train_dataset = datasets.concatenate_datasets([train_sg_dataset, train_sp_dataset, train_vp_dataset])
        
        dev_sg_dataset = datasets.Dataset.from_json(opt.sg_val_filepath)
    ...
    with accelerator.main_process_first():
        ...
        tokenized_dev_sg = dev_sg_dataset.map(
            preprocess_function,
            batched=True,
            num_proc=opt.preprocessing_num_workers,
            remove_columns=dev_sg_dataset.column_names,
            load_from_cache_file=not opt.overwrite_cache,
            desc="Running tokenizer on dev sg dataset",
        )
    ...
    model, optimizer, train_dataloader, dev_sg_dataloader, dev_sp_dataloader, dev_vp_dataloader, lr_scheduler  = accelerator.prepare(
        model, optimizer, train_dataloader, dev_sg_dataloader, dev_sp_dataloader, dev_vp_dataloader, lr_scheduler
    )
    ...
    while completed_steps < opt.num_training_steps:
        for step, batch in enumerate(train_dataloader):
            ...
            if completed_steps % opt.num_eval_steps == 0:
                ...
                dev_sg_pbar = tqdm(total=len(dev_sg_dataloader), disable=not is_local_main_process, desc="Evaluating sg performance..")
                
                for step, batch in enumerate(dev_sg_dataloader):
                    with torch.no_grad():
                        ...
                        if accelerator.num_processes > 1:
                            if step == len(dev_sg_dataloader) - 1:
                                decoded_preds = decoded_preds[: len(dev_sg_dataloader.dataset) - samples_seen]
                                decoded_labels = decoded_labels[: len(dev_sg_dataloader.dataset) - samples_seen]
                            else:
                ...
                if is_local_main_process:
                    ...
                    if opt.wandb_log:
                        ...
                        logger.info(f"At {completed_steps} training step, sql's eval_bleu = {metrics['score']}.")
