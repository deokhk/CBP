...
class MT5ForConditionalGenerationWithLP(MT5ForConditionalGeneration):
    ...
    # Copied from transformers.models.t5.modeling_t5.T5ForConditionalGeneration.__init__ with T5->MT5
    def __init__(self, config: MT5Config, num_lang=3):
        super().__init__(config)
        self.lp_network = torch.nn.Linear(config.d_model, num_lang)
    ...
    def forward(
        ...
        grad_reverse_lambda: Optional[float] = None,
    ) -> Union[Tuple[torch.FloatTensor], Seq2SeqLMOutput]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the sequence classification/regression loss. Indices should be in `[-100, 0, ...,
            config.vocab_size - 1]`. All labels set to `-100` are ignored (masked), the loss is only computed for
            labels in `[0, ..., config.vocab_size]`
        task_id (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            if task_id is 0, then we perform normal training, if task_id is 1 , then we perform adversarial training
        grad_reverse_lambda: (`float`, *optional*):
            lambda for grad reverse. Required to make the learning stable.
        Returns:
        ...
        lp_loss_Fct = torch.nn.CrossEntropyLoss(reduction='mean')
        if task_id == 1:
            # Gradient adversarial training with lp network 
            hidden_states = torch.mul(hidden_states, attention_mask.float().unsqueeze(-1))
            hidden_states = (hidden_states.sum(dim=1) / attention_mask.sum(dim=-1, keepdim=True))

            hidden_states = GradientReversalFunction.apply(hidden_states, grad_reverse_lambda)
            lp_logits = self.lp_network(hidden_states)
            loss = lp_loss_Fct(lp_logits, labels)
            return Seq2SeqLMOutput(
                loss=loss,
                logits=lp_logits,
                encoder_last_hidden_state=encoder_outputs.last_hidden_state,
