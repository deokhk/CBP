...
def compute_exact_match_metric_sp(predictions, references) -> Dict[str, Any]:
    foreign_key_maps = dict()
    for reference in references:
        if reference["db_id"] not in foreign_key_maps:
            foreign_key_maps[reference["db_id"]] = spider_evaluation.build_foreign_key_map(
    ...
    eval_res = []
    error_count = 0
    for prediction, reference in tqdm(zip(predictions, references), total=len(predictions), desc="Evaluating Exact match.."):
        turn_idx = reference.get("turn_idx", 0)
        # skip final utterance-query pairs
        if turn_idx < 0:
            continue
        try:
            res = evaluator.evaluate_one(reference["db_id"], reference["query"], prediction)
            eval_res.append(res["exact"])
